...
static void create_handler(struct intr_frame *f){
	...
	success = filesys_create(file,initial_size, FILE);
...
static void write_handler(struct intr_frame * f){
	...
	if(fd == 1){
		...
		lock_acquire(&file_lock);
		if(inode_get_file_type(file_get_inode(fi->file)) == DIR){
			f->eax = -1;
			unpin_frame(buffer);
			lock_release(&file_lock);
			return;
...
static void chdir_handler(struct intr_frame *f){
	...
	if(dir_lookup(dir, file_name, &inode) == false){
		if(strcmp(name, "/")==0){//change to root.
			//dir is just current thread's working directory at this point.
		...
		if(strcmp(name, "..")==0){//change to parent.
			//dir is just current thread's working directory at this point.
			dir = thread_current()->cur_dir;
			int parent = inode_get_parent_sector(dir_get_inode(dir));
			thread_current()->cur_dir = dir_open(inode_open(parent));
			dir_close(dir);
			unpin_frame(name);
			f->eax = true;
			return;
	...
	dir_close(dir);
	ASSERT(inode_get_file_type(inode) == DIR);
	dir = thread_current()->cur_dir;
...
static void mkdir_handler(struct intr_frame *f){
	...
	dir = *((char **)(f->esp+4));
	check_user_pointer(dir,0);
	check_ustack_boundaries(f,1);

	pin_frame(dir);
	success = filesys_create(dir, 16, DIR);
	unpin_frame(dir);
...
static void readdir_handler(struct intr_frame *f){
	...
	pin_frame(name);
	struct file_info * fi = search_file_info(fd);
	// struct inode * inode = file_get_inode(fi->file);
	// ASSERT(inode_get_file_type(inode)==DIR);//fd represents a directory.
	// struct dir * dir = dir_open(inode_reopen(inode));
	success = dir_readdir(fi->dir, name);
...
static void isdir_handler(struct intr_frame *f){
	...
	struct file_info * fi = search_file_info(fd);
	success = (inode_get_file_type(file_get_inode(fi->file))==DIR);
...
static void inumber_handler(struct intr_frame *f){
	...
	struct file_info * fi = search_file_info(fd);
	inode_number = inode_get_inumber(file_get_inode(fi->file));
...
static int add_file_info(struct file * f){
	...
	fi->dir = dir_open(file_get_inode(f));
